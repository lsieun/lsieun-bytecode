...
import lsieun.bytecode.verifier.structurals.LocalVariables;
import lsieun.bytecode.verifier.structurals.OperandStack;
import lsieun.utils.StringUtils;
import lsieun.utils.radix.HexUtils;

//[local variable][f_stack]
@SuppressWarnings("Duplicates")
public class StackMapFrameVisitor2 implements OpcodeVisitor {
    ...
    public void visit(MethodInfo methodInfo, ConstantPool constantPool) {
        ...
        frame = new Frame(max_locals, max_stack);
        ...
        if (!isStatic) {
            idx++;
            Type type = new ObjectType("this");
            frame.getLocals().set(0, type);
        }

        Type[] argumentTypes = TypeUtils.getArgumentTypes(descriptor);
        for (int i = 0; i < argumentTypes.length; i++) {
            Type t = argumentTypes[i];
            frame.getLocals().set(idx + i, t);
        }
        ...
        Map<Integer, Frame> map = new HashMap<Integer, Frame>();
        System.out.println(getLine());

        Instruction current = chain.start;
        while(current != null) {
            // （1）恢复原来的Stack状态
            Frame restoredState = map.get(Integer.valueOf(offset));
            ...
            if (current instanceof BranchInstruction) {
                ...
                if (current instanceof SelectInstruction) {
                    ...
                    for (int delta : indices) {
                        Frame s = snapshot();
                        map.put(Integer.valueOf(offset + delta), s);
                    }
                }
            }
    ...
    public String getLine() {
        ...
        String localStr = Arrays.toString(locals.getLocals());
        String stackStr = StringUtils.list2str(stack.getStack(), "[", "]", ",");
        if (stackStr == null) {
            stackStr = "[]";
        }

        String format = "%s %s";
        String line = String.format(format, localStr, stackStr);
        return line;
    }
    ...
    public void visitIINC(final IINC obj) {
        String name = OpcodeConst.getOpcodeName(obj.opcode);
        System.out.printf(IINC_FORMAT, offset, name, obj.index, obj.constValue, getLine());
    }
...
//    }
    ...
    public void visitNEWARRAY(final NEWARRAY obj) {
        ...
        System.out.printf(NEW_ARRAY_FORMAT, offset, name, atype, getLine(), type);
    }
