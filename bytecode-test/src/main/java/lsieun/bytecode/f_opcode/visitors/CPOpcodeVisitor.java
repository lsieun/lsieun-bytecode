...
public class CPOpcodeVisitor implements OpcodeVisitor {
    ...
    public void visitLDC(LDC obj) {
        ...
        switch (c.tag) {
            case CPConst.CONSTANT_String:
                final int i = ((ConstantString) c).string_index;
                c = constant_pool.getConstant(i);
                setValue(((ConstantUtf8) c).utf8Value);
                break;
            case CPConst.CONSTANT_Float:
                setValue(new Float(((ConstantFloat) c).floatValue));
                break;
            case CPConst.CONSTANT_Integer:
                setValue(Integer.valueOf(((ConstantInteger) c).intValue));
                break;
            case CPConst.CONSTANT_Class:
                final int nameIndex = ((ConstantClass) c).name_index;
                c = constant_pool.getConstant(nameIndex);
                setValue(new ObjectType(((ConstantUtf8) c).utf8Value));
                break;
    ...
    public void visitLDC2_W(LDC2_W obj) {
        final Constant c = constant_pool.getConstant(obj.index);
        switch (c.tag) {
            case CPConst.CONSTANT_Long:
                setValue(Long.valueOf(((ConstantLong) c).longValue));
                break;
            case CPConst.CONSTANT_Double:
                setValue(new Double(((ConstantDouble) c).doubleValue));
                break;
            default: // Never reached
                throw new RuntimeException("Unknown or invalid constant type at " + obj.index);
        }
